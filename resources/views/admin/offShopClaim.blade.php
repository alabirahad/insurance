...
<?php
        ...
        <table class="table">
            ...
            <tbody>
                @if (!$OfficeShopClaim->isEmpty())
                <?php $sl = 1;?>
                    @foreach ($OfficeShopClaim as $data)
                    <tr>
                        <th scope="row">{{$sl++}}</th>
                        ...
                        <td>
                            <form action="{{ URL::to('admin-off-shop-claim/' . $data->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                                <button type="button" class="table-action edit-btn">Show</button>
